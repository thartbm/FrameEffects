...
In the first panel (top left), we can see that the amount of motion seems to be overestimated for both the regular frame as well as the moving background strip. It seems close, but it could be perhaps a bit more overestimated for the regular frame. Let's test that statistically:
...
2.  ( 0) static (static relative to the screen)
3.  (+1) the same as that of the frame (these move with the frame)
