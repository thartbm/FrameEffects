...
4.  probes presented at different time points during the frame motion cycles (not just at the extreme time points, when the frame changes direction) changing their relative distance in frame coordinates

Motion of the frame:

5.  dot-pattern frames, and backgrounds to test if the edges of the frame matter, or the motion signals surrounding the probes
...
Set up the renv packages used in this project:

```{r eval=FALSE}
# this installs the versions of packages used here
# as recorded in the renv.lock file
# the first time, this could take a while
renv::restore()
```
...
There clearly is an effect of frame offset: larger offsets result in smaller illusions. This may interact with the direction of the offset and the size of the frame. Which we will first test with an ANOVA:
...
A large frame has a larger distance between frame edges and flashed probes when there is no offset, and perhaps a smaller distance at some other offsets than smaller frames. If the distance between frame edges and flashed probes is relevant for the effect (which is consistent with smaller effects for very large offsets) we could get overlapping curves if they the effects are corrected for distance between frame edges and flashed probes. However, neither of two approaches we used (the average distance and the smallest distance) worked. So that this does not seem to be a valid explanation.

## 2. Depth offsets

Here we test if offsets in depth between the frame and the flashed probes have any effect on illusion size. We could put the frame behind the probes, in front of the probes, or even between the probes. We try this by using red-blue cardboard glasses, creating anaglyph stimuli. As a control, we also present a frame with probes in the same plane of depth.
...
In this task, the frame is unchanged, and hence always presented at the screen depth. The probes are each presented as a red and blue dot. These would either have 0.5 dva offset in either direction (relative to the screen) or no offset. Without offset, they would be perceived in the same plane of depth as the frame, and with offset they would be perceived as in front of or behind the frame.

First, to test depth perception in our participants they perform a short task judging depth distances between two circles. The two circles have the same size as the probes, as well as the same vertical distance between them, and are offset in depth with the same offsets as used in the main task. One participant could not do this task, and also skipped the main task. Of the remaining 13 participants, 11 had a response criterion over 75% correct, and these were used for data analysis (although including all 13 doesn't meaningfully change the results).

```{r}
knitr::kable( getDepthControlData() )
```
...
First, we test if the brain uses prediction of frame position, i.e. is there an illusion in perceived probe position when the probes are flashed after the frame has disappeared? We also test the possibility of post-diction, where the probe positions could be retro-actively re-interpreted when the frame appears after the probes have been flashed on the screen.

This could require some build-up of expectations, so we try this with 1, 2 or 3 frame passes, with a few conditions we do not analyze here. And we test different shifts/offsets of timing.
...
The maximum stimulus duration would last up to 5 frame movement passes (1.5 cycles). We add at least 6 passes (3 cycles) in between presentations of the stimuli, or 7 passes if the stimulus consists of an odd number of passes.
...
We did also add single frame passes at 4 lower frame movement amplitudes, for the overall effect of time on task. Both probes were flashed during the single pass of the frame. These were each presented 4 times to the participants. Since the perceived offsets with both probes presented with the frame were indistinguishable from the perceived offsets in other tasks, this data is indeed included in the overall time-on-task analysis at the end of this report, but not analyzed further here.
...
The conditions with 1 probe flashed with the frame were still somewhat unclear, so we added conditions with one probe flashed during the first or last of 4 or 5 frame movement passes (and the other before/after). These were each also presented 4 times to the participants. However, they provide no additional insight: the effect is the same is in the other conditions with 1 probe flashed with the frame on the screen. So while the data is in the data repository for interested readers, they are not shown or discussed here.

Let's first look at the main data from this experiment.

```{r postdiction_fig, fig.width=5, fig.height=4}
fig5_prepost()
```

Here, the X-axis represents the time when the probes are flashed, relative to the onset/offset of the frame movement. On the left, the probes are flashed before the frame is on the screen (0 overlap), and on the right after the frame has disappeared (also 0 overlap).

Here we analyse this data, with a repeated-measures ANOVA on the illusion strength (percept) using the factors: overlap (number of probes flashed with the frame on screen), direction (pre vs. post) and number of passes (1, 2 or 3).

The timing of when the probes are flashed should have an effect, as it can be clearly seen in the figure. It doesn't look like there is a clear effect of the number of passes, but who knows. For the conditions where 1 probe was flashed simultaneously with the frame, the effect might also depend on the direction (so there could be a weak interaction).

```{r postdiction-ANOVA}
postdictionANOVA()
```

There is no effect of _passes_ (F(1.24,16.12)=3.44; p=.075) but there is still a main effect of _overlap_ (F(1.88,24.39)=310.67, p\<.001). There is no main effect of _direction_ (post or pre), but _overlap_ does interact with _direction_ (F(1.19,15.45)=5.98, p=.023). This is probably because of the difference in illusion strength between stimuli where either the second or the first flashed probed was shown simultaneously with the frame. We do 3 post-hoc t-tests to check this out.
...
There is no interaction however, suggesting that the effect of delays is the same in both frame types. This would mean identical slopes in the above linear fits, but to be fair they are pretty close. We'd probably need a better data set to test this.

## Conclusions on effects of time

The illusion is modulated in time. However, the temporal proximity seems to require at least one probe be perceived more or less simultaneously with the frame. This extends to the apparent motion frame. While the illusion is still there with lagged probes with a frame the moves with apparent motion, the frame is not present all the time, providing less positional information to reference the position of the probes to - even when the frame is present.
...
```{r background-motion-fig, fig.width=6.5, fig.height=7.7}
fig7_background()
```

In the first panel (top left), we can see that the amount of motion (weirdly) seems to be overestimated for both the regular frame as well as the moving background strip. It seems close, but it could be perhaps a bit more overestimated for the regular frame. Let's test that statistically:
...
Motion duration (and hence speed) has no effect and does not interact with stimulus type.

These are the average perceived motion amplitudes at 4 dva for both stimulus types (free viewing):

```{r}
motionPerceptionDescriptors()
```
We can compare each of these to zero, and compare them with each other.

```{r motion-perception-ttests}
motionperceptionTtests()
```

Of course, both are clearly different from zero, but they are also different from each other. With a background strip, the perceived motion is about 84% of the motion perceived in actual frames, with an absolute difference of 0.85 dva.

We now look at the illusion in the same stimuli. There are 2 extra conditions we test here. First, it's known that motion duration of the frame doesn't affect illusion strength, but this is not known for random dot backgrounds, so we include 5 motion durations here. Further, it could be that fixating peripherally, vs. free viewing (including looking directly at the stimulus) affects illusion strength. So for the frames, we have included trials with and without fixation instructions.
...
The tests confirm what we can see in the data (panel C of the above figure).

With frames the illusory offset is around 3.5 dva and with a dot background it is around 1.9 dva. That is, with a dot background we have about 53% of the illusory offset, while we have 85% of the perceived motion.
...
This paints the same picture as before, and confirms there is no effect of motion duration in either regular frames (as expected) nor in random dot backgrounds (F(2.3,29.94)=0.47, p=.653). There is an effect of condition (F(1.98,25.78)=53.88, p\<.001) but no interaction (F(4.01,52.25)=1.17, p=.335).

## 6. Internal frame motion

The previous experiment showed that having an actual frame, with edges, works much better than just a pattern of dots. But texture motion signals do still induce an effect. If frames, with edges, consist of patterns of dots that (could) move independently of the frame, that may affect the illusion strength.
...
If the internal motion has any effect on the strength of the illusion, we could expect the illusion strength to be lowest for the first condition, and highest for the last. With the third condition having about the same illusion strength as a classic frame (since the parts of an actual frame move with the frame, just like the dots in this stimulus).
...
Seems like very little effect of the internal motion on perceived frame motion, or on perceived offset. Although the frame where the dots move double the amount of the frame might have some decreased motion perception and increased illusion. We run two F-tests with internal dot motion (4 levels) as explanatory factor.
...
Now an F-test on illusion strength (with post-hocs):

```{r internal-motion-illusion-anova}
internalmotionIllusionANOVA()
```

Unexpectedly, there is an effect here (hence the post-hoc tests). The illusion is slightly stronger when the internal dots move double the frame motion, as compared to when they move opposite the frame motion (p=.007) or are static (p=.005). There are no other differences between conditions. The condition that is most similar to regular frames (dots "moving" with frame) is not different from any of the other conditions. That is, the effect of the texture motion we saw in the previous experiment is largely vetoed by the frame itself.

## 7. Frames moved by the participant

When people observe a frame being moved, their vision is passive, perhaps without much expectations. As seen in the pre-/post-diction analyses (results 3 here) there is no build-up of expectations over the course of 3 frame passes either. There could however, be an added efference-based expectation when participants move the frame themselves. We do a straightforward test here where people use a stylus on a drawing tablet with their right hand to move the frame left to right. They can simultaneously use the arrow keys on a keyboard to indicate their percept (although most participants seemed to alternate between first observing the stimulus, then adjusting the reference dots, repeating this until they were satisfied).

  We added two safeguards for this task. First, a cardboard slot was taped over the drawing tablet, keeping the stylus confined to the required horizontal regions, as well as ensuring that participants never had to look at the tablet or stylus to move it correctly. Second, to ensure roughly the same stimulus for all participants, auditory ticks were played as a kind of metronome, one tick every 633.3 ms. Participants were instructed that the sound of hitting the cardboard on either end should coincide with the metronome ticks as best as they could. Probes were flashed when participants were at the extremes.
...
So there is clearly an effect of amplitude, but no effect of interval and no interaction. This means we can conclude that there is no measurable decrease of illusion strength over the course of a 2 hour interval. The illusion seems not to be interpreted as an error by the visual system that then gets corrected over time.

# Figure export

This block outputs all above figures to various formats.

```{r fig-export, include=FALSE, eval=FALSE}
for (target in c('pdf', 'svg', 'png')) {
  
  fig3_offsets(target=target)
  fig4_depth(target=target)
  fig5_prepost(target=target)
  fig6_probelag(target=target)
  fig7_background(target=target)
  fig8_internalmotion(target=target)
  fig9_selfmotion(target=target)
  fig10_tasktime(target=target)
